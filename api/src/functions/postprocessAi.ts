...
function stripCodeFences(text: string): string {
  return text.replace(/```json\s*([\s\S]*?)\s*```/gi, "$1").replace(/```\s*([\s\S]*?)\s*```/g, "$1").trim();
}
...
export function isMetaResponseText(text: string): boolean {
  const lines = collectUsableLines(text);
...
export function postProcessExplainFromAi(raw: string, term: string): string | null {
  ...
  const jsonDetail = parsed && typeof parsed.detail === "string" ? parsed.detail.trim() : "";
  ...
  const termLine = lines.find((line) => line.toLowerCase().includes(term.toLowerCase()) && line.length >= 10);
  if (termLine) {
    return shorten(ensurePossibilityPhrase(termLine), 220);
  }
  ...
  return shorten(`この会議では「${term}」は ${sentence.replace(/[。！？!?]$/, "")} を指す可能性があります。`, 220);
}

export function postProcessNotesFromAi(raw: string, clickedTerms: string[]): string | null {
  const parsed = parseJsonFromText<Record<string, unknown>>(raw);
  const jsonNotes = parsed && typeof parsed.notes === "string" ? parsed.notes.trim() : "";
  if (jsonNotes && !isMetaResponseText(jsonNotes)) {
    return shorten(jsonNotes, 360);
  }
  ...
  const hasTerm = clickedTerms.some((t) => picked.toLowerCase().includes(t.toLowerCase()));
  if (hasTerm) {
    return shorten(picked, 360);
  }

  const tail = clickedTerms.length > 0 ? `主な確認語は ${clickedTerms.slice(0, 5).join("、")} です。` : "";
