...
function isOpenAiV1Endpoint(url: string): boolean {
  return normalizeUrl(url).toLowerCase().endsWith("/openai/v1");
}

function resolveCompatEnv(primary: string, alias: string): string {
  if (readEnv(primary)) return requireEnv(primary);
  if (readEnv(alias)) return requireEnv(alias);
  throw new Error(`Missing required environment variable: ${primary} (or ${alias})`);
}

function getAzureConfig(): AzureOpenAiConfig | null {
  if (!hasAnyEnv([...AZURE_CORE_ENV, "AZURE_OPENAI_API_VERSION"])) return null;
  ...
  if (isOpenAiV1Endpoint(endpoint)) return null;
  return { provider: "azure_openai", endpoint, apiKey, deployment, apiVersion };
}
...
async function callOpenAiCompat(
  ...
  if (wantsThinkingDisabled) requestBody.thinking = { type: "disabled" };
  ...
  if (!text)
    context.warn(
      `OpenAI-compatible empty content. payload keys=${Object.keys(payload).join(",")}`
    );
  return text;
}
...
export function parseJsonFromText<T>(text: string): T | null {
  ...
  const fenced =
    direct.match(/```json\s*([\s\S]*?)\s*```/i) ??
    direct.match(/```\s*([\s\S]*?)\s*```/i);
  if (fenced?.[1]) {
  ...
  if (objectLike?.[0]) {
    try { return JSON.parse(objectLike[0]) as T; } catch { /* continue */ }
  }

  const arrayLike = direct.match(/\[[\s\S]*\]/);
  if (arrayLike?.[0]) {
    try { return JSON.parse(arrayLike[0]) as T; } catch { /* continue */ }
  }
